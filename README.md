...
当前历史 SQL 被固化为 `baseline-2026-07-22` 单一基线，根变更日志最新为 V060。V042—V048 依次
落地插件控制面、PLUGIN-only、完整性冻结和显式主备路由；V049 增加 Manifest v2、SIMPLE Spec 和
编译事实；V050 以不可覆盖的静态事实种入 `generic-http:2.0.0`，V051 扩展 CallRecord 连接器错误码的持久化宽度，V052 绑定 CallRecord 的接口身份；V053—V057 分别固化管理权限、计费发布锁、操作日志租户范围、配置版本加密元数据和告警类型宽度，V058 修复 API Key 权限目录父级引用，V059 固化调用场景租户所有权，V060 分离精确缓存重放载荷与脱敏审计响应。后续变更必须追加独立 Liquibase
...
从 fresh PostgreSQL 开始，脚本会启动本地基础设施，应用 V001—V060 迁移，构建并启动六个服务和前端，种入隔离的 3 家厂商/2 个调用系统/2 类数据场景，然后通过 Gateway 验证审批、OpenAPI、CallRecord、Billing、审计和监控事实：

```bash
DEV_MVP_SKIP_BUILD=false rtk bash data-platform-test/test-fixtures/dev-mvp/verify-dev-closure.sh
```

成功后会在 `data-platform-test/test-fixtures/.runtime/dev-mvp-latest-report.json` 生成不含密钥的机器可读报告，报告必须显示 `schemaVersion=V060`、`pendingMigrations=0` 和 `status=passed`。V060 将精确的成功响应单独保存为仅供授权缓存重放的内部载荷，普通 CallRecord 查询仍只返回脱敏 `responseData`，旧行不从脱敏数据回填。失败运行会保留精确的隔离数据库、日志和进程状态用于排查；该夹具不会写入正式迁移。
...
四角色真实浏览器代表性验收、运行态缺口和清理边界见 [验收结果](docs/2026-08-31-real-browser-end-to-end-acceptance-results.md)；后续修复顺序与完成标准见 [整改方案](docs/2026-09-01-real-browser-acceptance-remediation-plan.md)。验收通过只证明 Dev 核心业务闭环，不替代尚未完成的安全矩阵和生产门禁。
...
| dev MVP 一键业务闭环 | ✅ fresh V060、3/2/2 夹具、持久 demo、真实浏览器、受保护清理和机器报告 v2 已通过；工作日定时 E2E 持续观察 | 2026-09-03 |
| 四角色真实浏览器整改复审 | ✅ P0/P1 技术整改、V060、P6、敏感扫描和受保护清理已通过；远端交付以最终 PR SHA 的 required CI 为门禁，产品停点和生产门禁未宣称完成 | 2026-09-03 |
...
- [2026-08-31 真实浏览器全链路验收结果](docs/2026-08-31-real-browser-end-to-end-acceptance-results.md)
- [2026-09-01 真实浏览器验收问题整改方案](docs/2026-09-01-real-browser-acceptance-remediation-plan.md)
