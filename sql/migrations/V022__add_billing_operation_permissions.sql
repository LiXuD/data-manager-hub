...
VALUES
    ...
    ('billing:reconcile', '计费管理-账单对账', 'button', '/billing/reconciliation', 0, 43, '导入厂商账单、运行和查看对账结果',       'active'),
    ('billing:view-all', '计费管理-跨租户查看', 'api', '/billing',               0, 44, '查看和导出所有租户的账单',               'active')
...
WHERE LOWER(r.role_code) = 'admin'
  AND p.permission_code IN ('billing:manage', 'billing:reverse', 'billing:reconcile', 'billing:view-all')
