...
- 当前最新迁移为 **V060**；新迁移一律从 **V061** 起顺延编号。
...
| V059 | 为调用场景建立租户所有权、租户复合唯一性和运行时查询索引；无法明确归属的存量场景原子 HALT | U059 始终拒绝移除调用场景租户范围 |
| V060 | 将可复用成功响应与脱敏审计响应分离；普通 CallRecord 查询不读取精确缓存载荷，旧行不回填 | U060 始终拒绝原地删除精确缓存载荷 |
...
V053—V060 是本轮安全和运行正确性收尾的独立前向迁移：V053—V057 分别固化管理权限矩阵、计费发布串行化锁、操作日志租户范围、配置版本加密元数据和告警类型宽度；每个 changeset 都保持已执行历史不可逆。V058 不改写 V053 的执行记录，只按稳定的 `permission_code` 修复 API Key 权限目录父级，fresh、升级和重复执行均必须保持幂等。V059 在启用场景业务读写前强制补齐租户所有权；无法从现有事实明确归属的存量行直接 HALT，禁止猜测归属。V060 新增只供授权缓存重放使用的精确成功响应载荷，普通查询默认排除该列，且不从可能已脱敏的历史 `response_data` 回填。U053—U060 均为拒绝式 rollback，真实恢复使用经过验证的备份或新增 forward-recovery changeset。
