...
- 当前最新迁移为 **V059**；新迁移一律从 **V060** 起顺延编号。
...
V053—V057 是本轮安全和运行正确性收尾的独立前向迁移：分别固化管理权限矩阵、计费发布串行化锁、操作日志租户范围、配置版本加密元数据和告警类型宽度；每个 changeset 都保持已执行历史不可逆。V058 不改写 V053 的执行记录，只按稳定的 `permission_code` 修复 API Key 权限目录父级，fresh、升级和重复执行均必须保持幂等。V059 在启用场景业务读写前强制补齐租户所有权；无法从现有事实明确归属的存量行直接 HALT，禁止猜测归属。U053—U059 均为拒绝式 rollback，真实恢复使用经过验证的备份或新增 forward-recovery changeset。
